...
class StockMonitor:
    def __init__(self):
        ...
        self.global_notification_enabled = config_manager.get_global_notification_enabled()
        # 注册为配置管理器的观察者
        ...
        try:
            if change_type == 'settings_update':
                ...
                if 'global_notification_enabled' in data:
                    self.global_notification_enabled = data['global_notification_enabled']
                    logger.info(f"全局通知开关已更新为: {self.global_notification_enabled}")
            elif change_type == 'stock_add':
                # 添加新股票
                new_stock = data
                # 检查股票是否已存在
                existing_stock = next((s for s in self.stocks if s['code'] == new_stock['code']), None)
                if not existing_stock:
                    self.stocks.append(new_stock)
                    logger.info(f"新股票 {new_stock['name']}({new_stock['code']}) 已添加到监控列表")
                ...
                # 清除该股票的相关通知状态
                keys_to_remove = [key for key in self.notified_stocks.keys() if key.startswith(f"{code}_")]
        ...
        try:
            while True:
                ...
                if is_market_open(current_time):
                    try:
                        logger.info("=" * 50)
                        logger.info(f"开始新一轮股票价格检查 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                        
                        # 计算每只股票的检查间隔时间
                        if len(self.stocks) > 0:
                            per_stock_interval = self.check_interval / len(self.stocks)
                        else:
                            per_stock_interval = self.check_interval  # 如果没有股票，使用完整间隔
                        ...
                        for i, stock in enumerate(self.stocks):
                            ...
                            elif status in ["low_alert_sent", "high_alert_sent", "limit_up_alert_sent", "limit_down_alert_sent", "key_price_alert_sent", "change_pct_alert_sent"]:
                                logger.info(f"已发送 {status} 给 {stock['name']}({stock['code']})")
                            elif status == "normal":
                                logger.debug(f"{stock['name']}({stock['code']}) 价格正常")
                            elif status == "error":
                                logger.error(f"检查 {stock['name']}({stock['code']}) 时发生错误")
                            
                            # 如果不是最后一只股票，等待分配的时间间隔
                            if i < len(self.stocks) - 1:
                                logger.debug(f"等待 {per_stock_interval:.2f} 秒后检查下一只股票")
                    ...
                    if current_time.weekday() < 5:  # 0是周一，4是周五
                        ...
                        elif current_time.time() < datetime.strptime("09:30", "%H:%M").time():
        ...
        try:
            ...
            if stock['low_alert_price'] and current_price <= stock['low_alert_price']:
                ...
                if not self.notified_stocks.get(alert_key):
                    ...
                    next_interval = timedelta(hours=2**(alert_count-1)) if alert_count >= 1 else timedelta(hours=1)
                    if datetime.now() - last_alert_time >= next_interval:
                        should_send = True

                if should_send:
                    logger.info(f"发现低价警报: {stock['name']}({stock['code']}) 当前价格 {current_price} <= 目标价格 {stock['low_alert_price']}")
                    # 检查全局和单个股票的消息发送开关
                    #if self.global_notification_enabled and self.stock_notification_enabled.get(stock['code'], True):
                    if self.global_notification_enabled and stock.get('notification_enabled', True):
                        ...
                        if result['success']:
                            ...
                            if self.notified_stocks.get(alert_key):
                                ...
                                count = 1
                            self.notified_stocks[alert_key] = {'time': datetime.now(), 'count': count}
            ...
            if stock['high_alert_price'] and current_price >= stock['high_alert_price']:
                ...
                if should_send:
                    logger.info(f"发现高价警报: {stock['name']}({stock['code']}) 当前价格 {current_price} >= 目标价格 {stock['high_alert_price']}")
                    # 检查全局和单个股票的消息发送开关
                    if self.global_notification_enabled and stock.get('notification_enabled', True):
                        result = send_stock_alert(
                            stock['name'], 
                ...
                for key_price_info in stock['key_price_alerts']:
                    ...
                    if abs(current_price - key_price) / key_price <= 0.01:
                        ...
                        if not self.notified_stocks.get(alert_key):
                            ...
                            alert_count = self.notified_stocks[alert_key]['count'] # 已发送次数
                            # 计算下次发送间隔：1小时，2小时，4小时...，以2的幂次递增
                            next_interval = timedelta(hours=2**(alert_count-1)) if alert_count >= 1 else timedelta(hours=2)
                            if datetime.now() - last_alert_time >= next_interval:
                                should_send = True

                        if should_send:
                            logger.info(f"发现关键价位警报: {stock['name']}({stock['code']}) 当前价格 {current_price} 接近{price_type} {key_price}")
                            # 检查全局和单个股票的消息发送开关
                            if self.global_notification_enabled and stock.get('notification_enabled', True):
                                result = send_stock_alert(
                                    stock['name'], 
                ...
                if change_pct is not None:
                    for change_pct_info in stock['change_pct_alerts']:
                        ...
                        if should_alert:
                            ...
                            if should_send:
                                alert_direction = "上涨" if change_pct > 0 else "下跌" if change_pct < 0 else "无变化"
                                logger.info(f"发现涨跌幅警报: {stock['name']}({stock['code']}) {alert_direction}{abs(change_pct):.2f}%，超过阈值{threshold_pct}%")
                                # 检查全局和单个股票的消息发送开关
                                if self.global_notification_enabled and stock.get('notification_enabled', True):
                                    result = send_stock_alert(
                                        stock['name'], 
                                    ...
                                    if result['success']:
                                        ...
                                        if self.notified_stocks.get(alert_key):
                                            ...
                                            count = 1
                                        self.notified_stocks[alert_key] = {'time': datetime.now(), 'count': count}
                                    return current_price, f"change_pct_alert_sent_{pct_type}"
                                else:
                                    logger.info(f"消息发送已关闭: {stock['name']}({stock['code']})")
                                    return current_price, "normal"
            ...
            if stock['limit_alert']:
                ...
                if change_pct is not None:
                    ...
                    if change_pct >= 9.8:
                        ...
                        if should_send:
                            logger.info(f"发现涨停警报: {stock['name']}({stock['code']}) 涨幅 {change_pct}%")
                            # 检查全局和单个股票的消息发送开关
                            if self.global_notification_enabled and stock.get('notification_enabled', True):
                                result = send_stock_alert(
                                    stock['name'], 
                    ...
                    # 检查跌停（跌幅超过-9.8%）
                        ...
                        if should_send:
                            logger.info(f"发现跌停警报: {stock['name']}({stock['code']}) 跌幅 {change_pct}%")
                            # 检查全局和单个股票的消息发送开关
                            if self.global_notification_enabled and stock.get('notification_enabled', True):
                                result = send_stock_alert(
                                    stock['name'], 
    ...
    def send_market_summary_report(self, report_type):
        ...
        """
        try:
            logger.info(f"发送{('开市后' if report_type == 'open_report' else '闭市后')}汇总报告")
            report_title = f"【{'开市后' if report_type == 'open_report' else '闭市后'}汇总报告】"
            report_content = f"【{'开市后' if report_type == 'open_report' else '闭市后'}市场汇总报告】\n\n"
            report_content += f"报告时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            
            for stock in self.stocks:
                # 检查股票配置中的关键字段是否为None
                stock_name = stock.get('name') if isinstance(stock.get('name'), str) else (stock['name'] if 'name' in stock else '未知股票')
                stock_code = stock.get('code') if isinstance(stock.get('code'), str) else (stock['code'] if 'code' in stock else '未知代码')
                ...
                if current_price is not None:
                    ...
                    report_content += f"股票名称: {stock_name} ({stock_code}) - 价格获取失败\n\n"
            
            # 发送汇总报告
            send_system_notification(report_title, report_content)
            logger.info(f"{('开市后' if report_type == 'open_report' else '闭市后')}汇总报告发送完成")
        except Exception as e:
...
def is_market_open(current_time=None):
    ...
    afternoon_session = afternoon_open_minutes <= current_minutes <= afternoon_close_minutes
...
def is_market_open_time_specific(current_time=None):
    ...
    # 计算开市后5分钟和闭市后5分钟的时间
    open_plus_5 = datetime.combine(current_time.date(), open_time) + timedelta(minutes=5)
    close_plus_5 = datetime.combine(current_time.date(), close_time) + timedelta(minutes=5)

    # 检查是否为开市后5分钟（允许1分钟的误差范围）
    if (open_plus_5 - timedelta(minutes=1)).time() <= current_time.time() <= (open_plus_5 + timedelta(minutes=1)).time():
        return 'open_report'
    # 检查是否为闭市后5分钟（允许1分钟的误差范围）
    elif (close_plus_5 - timedelta(minutes=1)).time() <= current_time.time() <= (close_plus_5 + timedelta(minutes=1)).time():
        return 'close_report'
    else:
        return None
