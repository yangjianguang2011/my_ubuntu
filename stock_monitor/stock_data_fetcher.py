...
def _fetch_historical_data(stock_code, days=365):
    ...
    cached_data = get_stock_cached_data(cache_key, cache_duration=3600*24)  # 缓存 1 天
    if cached_data is not None:
        return cached_data
    ...
    try:
        ...
        if len(stock_code) == 5:
            ...
            if df is None or df.empty:
                try:
                    logger.info(f"嘗試使用港股日線數據接口獲取 {stock_code} 歷史數據...")
                    ...
                    error_msg = f"港股日線數據接口獲取股票 {stock_code} 歷史數據失敗：{str(e)}"
                    logger.warning(error_msg)
            ...
            if df is not None and not df.empty:
                ...
                if '日期' in df.columns:
                    ...
                    # 只重命名存在的列
                    cols_to_rename = {k: v for k, v in standard_cols.items() if k in df.columns}
                    if cols_to_rename:
            ...
            try:
                ...
                end_date = datetime.now()
                #start_date = end_date - timedelta(days=days * 2)  # 多获取一些，以防停牌等情况
                start_date = end_date - timedelta(days=days) - timedelta(days=7)  # 确保至少有 days 天的数据
                start_date_str = start_date.strftime("%Y%m%d")
                end_date_str = end_date.strftime("%Y%m%d")
                # 使用纯数字代码，不带 sh/sz 前缀，带明确的日期参数
                df = ak.stock_zh_a_hist(symbol=stock_code.zfill(6), period="daily", adjust="", start_date=start_date_str, end_date=end_date_str)
                if df is not None and not df.empty:
            ...
            if df is None or df.empty:
                try:
                    ...
                    market_prefix = "sh" if stock_code.zfill(6).startswith('6') else "sz"
                    df = ak.stock_zh_a_daily(symbol=f"{market_prefix}{stock_code.zfill(6)}", adjust="")
                    if df is not None and not df.empty:
                        logger.info(f"日線數據接口獲取 {stock_code} 歷史數據成功")
                except Exception as e:
                    error_msg = f"日線數據接口獲取股票 {stock_code} 歷史數據失敗：{str(e)}"
                    logger.warning(error_msg)
                    error_messages.append(error_msg)
            ...
            if df is None or df.empty:
                try:
                    ...
                    df = ak.stock_zh_a_hist_tx(symbol=stock_code.zfill(6), start_date="", end_date="", adjust="")
                    if df is not None and not df.empty:
            ...
            if df is None or df.empty:
                try:
                    logger.info(f"嘗試使用 easyquotation daykline 接口獲取 {stock_code} 歷史數據...")
                    quotation = easyquotation.use('daykline')
                    # 获取 365 天的数据
                    data = quotation.real([stock_code.zfill(6)])
                    if data and stock_code.zfill(6) in data and data[stock_code.zfill(6)]:
                        # 转换为 DataFrame
                        kline_data = data[stock_code.zfill(6)]
                        df = pd.DataFrame(kline_data, columns=['date', 'open', 'close', 'high', 'low', 'volume'])
                        logger.info(f"easyquotation daykline 接口獲取 {stock_code} 歷史數據成功")
                except Exception as e:
...
def _calculate_technical_indicators(historical_data):
    ...
    df['bias_ma60'] = ((df['close'] - df['ma60']) / df['ma60'] * 100).round(2) if 'ma60' in df.columns else np.nan
    ...
    indicators = {
        ...
        'ma60': round(latest['ma60'], 2) if 'ma60' in latest and pd.notna(latest['ma60']) else None,
        ...
        'bias_ma5': round(latest['bias_ma5'], 2) if pd.notna(latest['bias_ma5']) else None,
        'bias_ma20': round(latest['bias_ma20'], 2) if pd.notna(latest['bias_ma20']) else None,
        'bias_ma60': round(latest['bias_ma60'], 2) if pd.notna(latest['bias_ma60']) else None,
...
def get_enhanced_stock_info(stock):
    ...
    try:
        ...
        logger.info(f"增强的股票 {stock_name}({stock_code}) 信息获取完成: {enhanced_info}")
