...
def get_stock_analyst_history_tracking(stock_code, days=360):
    """从长期存储中获取股票历史分析师跟踪数据"""
    try:
...
def _fetch_analyst_stocks(analyst_id, analyst_name, indicator="最新跟踪成分股", add_delay=True):
    ...
    try:
        ...
        logger.info(f"分析师 {analyst_name}({analyst_id}) 获取到 {len(analyst_stocks)} 只 {indicator} 股票")
...
def get_analyst_rank_data(period="2026"):
    ...
    try:
        ...
        logger.info(f"获取到 {len(analyst_rank_data)} 条分析师排行榜数据")
...
def get_combined_analyst_data(top_analysts=20, top_stocks=50, period="3个月"):
    """
    ...
    try:
        ...
        analyst_rank_data = get_analyst_rank_data(period)
        ...
        # 保存历史数据 - 在获取完数据后自动保存（只保存重点关注的股票）
        from datetime import date
        today = date.today().strftime('%Y-%m-%d')
        logger.info("开始保存历史分析师数据...")
...
def get_analyst_focus_stocks(top_analysts=20, top_stocks=50, period="3个月"):
    """
    ...
    :return: 分析师重点关注股票列表
    """
    cache_key = f"analyst_focus_stocks_{period}_{top_analysts}_{top_stocks}"
    cached_data = get_analyst_cached_data(cache_key)
    if cached_data is not None:
    ...
    combined_data = get_combined_analyst_data(top_analysts, top_stocks, period)
    result = {
...
def get_latest_analyst_tracking(top_analysts=20, top_stocks=50, period="3个月"):
    """
    ...
    combined_data = get_combined_analyst_data(top_analysts, top_stocks, period) # 使用传入的top_stocks参数
...
def get_recently_updated_stocks(days=30, indicator="最新跟踪成分股"):
    """
    获取最近更新的股票列表
    ...
    try:
        ...
        # 获取分析师排行榜数据
        analyst_rank_data = get_analyst_rank_data(period="3个月")
...
def get_stock_growth_comparison(symbol="SZ000895"):
    """
    获取指定股票的成长对比数据
    :param symbol: 股票代码，默认"SZ000895"
    :return: 成长对比数据的DataFrame
    """
    # 格式化股票代码
    formatted_symbol = _format_stock_symbol(symbol)
    cache_key = f"stock_growth_comparison_{formatted_symbol}"
    cached_data = get_analyst_cached_data(cache_key)
    if cached_data is not None:
        logger.info(f"从缓存返回股票 {formatted_symbol} 的成长对比数据")
        return cached_data

    try:
        logger.info(f"开始获取股票 {formatted_symbol} 的成长对比数据")
        import akshare as ak
        df = ak.stock_zh_growth_comparison_em(symbol=formatted_symbol)
        result = df.to_dict('records')
        set_analyst_cache_data(cache_key, result)
        logger.info(f"股票 {formatted_symbol} 的成长对比数据获取完成，共{len(result)}条记录")
        return result
    except Exception as e:
        logger.error(f"获取股票 {formatted_symbol} 的成长对比数据时出错: {str(e)}")
        return []


def get_stock_valuation_comparison(symbol="SZ000895"):
    """
    获取指定股票的估值对比数据
    :param symbol: 股票代码，默认"SZ00895"
    :return: 估值对比数据的DataFrame
    """
    # 格式化股票代码
    formatted_symbol = _format_stock_symbol(symbol)
    cache_key = f"stock_valuation_comparison_{formatted_symbol}"
    cached_data = get_analyst_cached_data(cache_key)
    if cached_data is not None:
        logger.info(f"从缓存返回股票 {formatted_symbol} 的估值对比数据")
        return cached_data

    try:
        logger.info(f"开始获取股票 {formatted_symbol} 的估值对比数据")
        import akshare as ak
        df = ak.stock_zh_valuation_comparison_em(symbol=formatted_symbol)
        result = df.to_dict('records')
        set_analyst_cache_data(cache_key, result)
        logger.info(f"股票 {formatted_symbol} 的估值对比数据获取完成，共{len(result)}条记录")
        return result
    except Exception as e:
        logger.error(f"获取股票 {formatted_symbol} 的估值对比数据时出错: {str(e)}")
        return []


def get_stock_dupont_comparison(symbol="SZ000895"):
    """
    获取指定股票的杜邦对比数据
    :param symbol: 股票代码，默认"SZ0895"
    :return: 杜邦对比数据的DataFrame
    """
    # 格式化股票代码
    formatted_symbol = _format_stock_symbol(symbol)
    cache_key = f"stock_dupont_comparison_{formatted_symbol}"
    cached_data = get_analyst_cached_data(cache_key)
    if cached_data is not None:
        logger.info(f"从缓存返回股票 {formatted_symbol} 的杜邦对比数据")
        return cached_data

    try:
        logger.info(f"开始获取股票 {formatted_symbol} 的杜邦对比数据")
        import akshare as ak
        df = ak.stock_zh_dupont_comparison_em(symbol=formatted_symbol)
        result = df.to_dict('records')
        set_analyst_cache_data(cache_key, result)
        logger.info(f"股票 {formatted_symbol} 的杜邦对比数据获取完成，共{len(result)}条记录")
        return result
    except Exception as e:
        logger.error(f"获取股票 {formatted_symbol} 的杜邦对比数据时出错: {str(e)}")
        return []


def get_stock_peer_comparison_all(symbol="SZ000895"):
    """
    获取指定股票的所有同行比较数据（成长性、估值、杜邦分析）
    :param symbol: 股票代码，默认"SZ00895"
    :return: 包含所有比较数据的字典
    """
    # 格式化股票代码
    formatted_symbol = _format_stock_symbol(symbol)
    cache_key = f"stock_peer_comparison_all_{formatted_symbol}"
    cached_data = get_analyst_cached_data(cache_key)
    if cached_data is not None:
        logger.info(f"从缓存返回股票 {formatted_symbol} 的所有同行比较数据")
        return cached_data

    try:
        logger.info(f"开始获取股票 {formatted_symbol} 的所有同行比较数据")
        growth_data = get_stock_growth_comparison(formatted_symbol)
        valuation_data = get_stock_valuation_comparison(formatted_symbol)
        dupont_data = get_stock_dupont_comparison(formatted_symbol)
        
        result = {
            'growth_comparison': growth_data,
            'valuation_comparison': valuation_data,
            'dupont_comparison': dupont_data
        }
        set_analyst_cache_data(cache_key, result)
        logger.info(f"股票 {formatted_symbol} 的所有同行比较数据获取完成")
        return result
    except Exception as e:
        logger.error(f"获取股票 {formatted_symbol} 的所有同行比较数据时出错: {str(e)}")
        return {
            'growth_comparison': [],
...
if __name__ == "__main__":
    ...
    try:
        ...
        # print("\n2. 测试获取组合分析师数据:")
        # combined_data = get_combined_analyst_data(20, 50, "3个月")  # 减少数量以便快速测试
        # print(combined_data)
        ...
        #all_recent_stocks = get_recently_updated_stocks(days=30, indicator="历史跟踪成分股")
        ...
        result = get_combined_analyst_data(20, 50, '3个月')
        print(f"result: {result}")
