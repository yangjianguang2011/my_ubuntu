...
class ConfigManager:
    def __init__(self):
        # 数据库路径
        db_dir = gconfig.get('database_dir', './database')
    ...
    def _init_db(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_stock_code ON stock_config(stock_code)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_config_key ON stock_config(config_key)')
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for key, value in stock.items():
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?)
                    ''', (stock['code'], key, json.dumps(value), datetime.now().isoformat()))
    ...
    def update_settings(self, updates):
        ...
        with self.pending_changes_lock:
            self.pending_changes.append(('system_settings', updates, datetime.now().isoformat()))
    ...
    def set_market_times(self, open_time, close_time):
        ...
        self.update_settings({
            'market_open_start': open_time,
            'market_open_end': close_time
        })
    ...
    def add_stock(self, stock_info):
        ...
        # 记录变更，用于后台同步
        with self.pending_changes_lock:
            self.pending_changes.append(('add_stock', stock_info, datetime.now().isoformat()))
    ...
    def update_stock(self, code, updates):
        ...
        # 记录变更，用于后台同步
        with self.pending_changes_lock:
            self.pending_changes.append(('update_stock', {'code': code, 'updates': updates}, datetime.now().isoformat()))
    ...
    def delete_stock(self, code):
        ...
        # 记录变更，用于后台同步
        with self.pending_changes_lock:
            self.pending_changes.append(('delete_stock', {'code': code}, datetime.now().isoformat()))
    ...
    def _sync_to_file(self):
        ...
        logger.info(f"系统设置更新: {len(system_settings_updates)}, 股票更新: {len(stocks_to_update)}, 股票删除: {len(stocks_to_delete)}")
