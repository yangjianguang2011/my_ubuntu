...
class CookieHandler:
    ...
    def __init__(self, cookies_file: str = "xueqiu_cookies.txt"):
        ...
        # 导入增强版Cookies管理器
        try:
            from enhanced_cookie_handler import EnhancedCookieHandler
            self.enhanced_handler = EnhancedCookieHandler(cookies_file=cookies_file)
        except ImportError:
            logger.warning("无法导入增强版Cookies处理器，使用标准版本")
            self.enhanced_handler = None

    def load_cookies(self, driver: webdriver.Chrome) -> bool:
        """从 TXT 文件加载 cookies（支持标准和简单格式）"""
        # 如果有增强版处理器，优先使用它
        if self.enhanced_handler:
            return self.enhanced_handler.load_and_validate_cookies(driver)
        ...
        if Path(self.cookies_file).exists():
            try:
                ...
                with open(self.cookies_file, "r", encoding="utf-8") as f:
                    ...
                    if content:
                        # 检查是否为标准 Netscape 格式（包含制表符分隔的多列）
                        lines = content.split("\n")
                        ...
                        for line in lines:
                            ...
                            if line and not line.startswith("#") and "\t" in line:
                                parts = line.split("\t")
                                if len(parts) >= 7:  # Netscape 格式至少 7 列
                        ...
                        if is_netscape_format:
                            ...
                            for cookie_str in content.split(";"):
                                ...
                                if "=" in cookie_str and (
                                    "xueqiu" in cookie_str or "snowball" in cookie_str
                                ):
                                    try:
                        ...
                        logger.info(f"Cookies 已从 {self.cookies_file} 加载")
                        return True
            except Exception as e:
    ...
    def save_cookies(self, driver: webdriver.Chrome) -> None:
        ...
        # 如果有增强版处理器，使用其安全保存方法
        if self.enhanced_handler:
            success = self.enhanced_handler.save_cookies_safely(driver)
            if success:
                logger.info("Cookies 已通过增强版处理器安全保存")
            else:
                logger.warning("增强版处理器拒绝保存无效Cookies，使用原有逻辑")
                self._save_cookies_legacy(driver)
        else:
            self._save_cookies_legacy(driver)

    def _save_cookies_legacy(self, driver: webdriver.Chrome) -> None:
        """原有的保存cookies方法（保留作为后备）"""
        try:
            cookies = driver.get_cookies()
            # 过滤出 xueqiu.com 的 cookies
            cookie_pairs = []
            for cookie in cookies:
                if "domain" in cookie and "xueqiu" in cookie.get("domain", ""):
                    cookie_pairs.append(f"{cookie['name']}={cookie['value']}")

            # 保存为单行文本格式
            with open(self.cookies_file, "w", encoding="utf-8") as f:
                f.write("; ".join(cookie_pairs))
            logger.info(f"Cookies 已保存到 {self.cookies_file} (TXT 格式)")
        except Exception as e:
            ...
            try:
                ...
                for card in author_cards:
                    try:
                        ...
                        if author_info and author_info["id"] not in [
                            a["id"] for a in authors
                        ]:
                            authors.append(author_info)
                            new_authors_this_page += 1
                            logger.info(
                                f"  找到作者: {author_info['name']} (ID: {author_info['id']})"
                            )
                ...
                logger.info(
                    f"第 {page_num} 页获取到 {new_authors_this_page} 个新作者，当前总计 {len(authors)} 个"
                )
    ...
    def _find_author_cards(self, driver: webdriver.Chrome) -> List[WebElement]:
        ...
        for selector in css_selectors:
            try:
                ...
                if cards and len(cards) > 0:
                    logger.info(
                        f"使用 CSS 选择器 '{selector}' 找到 {len(cards)} 个用户卡片"
                    )
                    return cards
            except Exception:
                continue
        ...
        for selector in xpath_selectors:
            try:
                ...
                if cards and len(cards) > 0:
                    logger.info(
                        f"使用 XPath 选择器 '{selector}' 找到 {len(cards)} 个用户卡片"
                    )
                    return cards
            except Exception:
                continue
    ...
    def _extract_author_info(self, card: WebElement) -> Optional[Dict[str, str]]:
        ...
        if link and name:
            author_id = (
                link.split("/u/")[-1].split("?")[0]
                if "/u/" in link
                else link.split("/")[-1]
            )
            return {"id": author_id, "name": name, "link": link}
        return None
    ...
    def _verify_login_status(self, driver: webdriver.Chrome) -> bool:
        ...
        try:
            ...
            if any(keyword in page_text.lower() for keyword in ['login', '登录', 'signin', 'sign in']):
                logger.warning("检测到登录提示，登录状态验证失败")
                return False
            ...
            if any(keyword in title for keyword in ['login', '登录', 'signin']):
                logger.warning("检测到登录页面标题，登录状态验证失败")
                return False
...
class ArticleExtractor:
    ...
    def get_author_articles(
        self, author: Dict[str, str], max_pages: int = 50
        ...
        while page_num <= max_pages:
            ...
            try:
                ...
                for element in article_elements:
                    try:
                        ...
                        article_data = self._extract_article_data(
                            element, author, driver
                        )
                        if article_data and article_data["content"]:
                            # 检查时间过滤（仅限update-today模式）
                            if self.update_today and not self._is_today_article(
                                article_data["time"]
                            ):
                                continue

                            # 保存文章到按月归档的文件
                            filepath = self._get_month_file_path(
                                article_data, author_id, author_name
                            )
                ...
                logger.info(
                    f"  [完成] 第 {page_num} 页保存 {new_articles_this_page} 篇"
                )
    ...
    def _extract_article_data(
        self, element: WebElement, author: Dict[str, str], driver: webdriver.Chrome
        ...
        try:
            ...
            for link_elem in link_elements:
                ...
                # 识别文章链接：/数字/数字 格式
                url_path = href.replace("https://xueqiu.com", "").replace(
                    "http://xueqiu.com", ""
                )
                ...
                if len(parts) >= 3 and parts[1].isdigit() and parts[2].isdigit():
                    article_link = (
                        href if href.startswith("http") else "https://xueqiu.com" + href
                    )
                    break
            ...
            for selector in time_selectors:
                try:
                    ...
                    if time_text and len(time_text) < 50:
                        has_date_format = bool(
                            re.search(
                                r"\d{4}-\d{1,2}-\d{1,2}|\d{1,2}-\d{1,2}|小时前|分钟前|昨天|今天|刚刚|修改于",
                                time_text,
                            )
                        )
        ...
        # ========== 第五步：增强内容提取（处理展开按钮等） ==========
        content = self._enhance_content_extraction(
            element, content, article_link, driver
        )
        ...
        return {
            ...
            "time": time_text,
            "link": (
                article_link if article_link else f"https://xueqiu.com/{author['id']}"
            ),
        }

    def _enhance_content_extraction(
        self,
        element: WebElement,
        content: str,
        article_link: str,
        ...
        try:
            ...
            try:
                active_tab = driver.find_element(
                    By.CSS_SELECTOR, "div.timeline__tab__tags a.active"
                )
            ...
            tab_selectors = [
                ...
                (
                    By.XPATH,
                    '//div[contains(@class, "timeline__tab__tags")]//a[contains(text(), "原发")]',
                ),
    ...
    def _get_month_file_path(
        self, article_data: Dict[str, str], author_id: str, author_name: str
    ) -> Path:
        """获取月份文件路径
    ...
    def _save_article_to_file(
        self, filepath: Path, article_data: Dict[str, str], author: Dict[str, str]
    ) -> None:
        """保存文章到HTML文件（按月归档）
        ...
        try:
            ...
            html_content += (
                f"      <span class='publish-time'>发布时间: {absolute_time}</span>\n"
            )
            html_content += f"    </div>\n"
            html_content += f"  </div>\n"
...
"""
            ...
            for author, entries in authors_dict.items():
                ...
                html_content += (
                    f'            <h2 class="author-title">作者: {author}</h2>\n'
                )
...
class ScraperOrchestrator:
    ...
    def __init__(self, headless: bool = False, output_dir: str = ""):
        ...
        self.output_dir = get_path("crawlers", "xueqiu_data_dir", fallback=output_dir)
        self.author_extractor = AuthorExtractor(self.browser_manager, self.output_dir)
        self.article_extractor = ArticleExtractor(
            self.browser_manager, self.author_extractor, self.output_dir
        )
    ...
    def run(self, test_mode: bool = False, skip_login: bool = False) -> None:
        ...
        try:
            ...
            if authors_file.exists():
                try:
                    with open(authors_file, "r", encoding="utf-8") as f:
                        authors = json.load(f)
                    logger.info(
                        f"从 {authors_file} 读取到 {len(authors)} 个作者（可手动编辑该文件）"
                    )
                except Exception as e:
            ...
            for i, author in enumerate(authors_to_process):
                logger.info(f"\n{'='*60}")
                logger.info(
                    f"正在处理第 {i+1}/{len(authors_to_process)} 个作者: {author['name']}"
                )
                logger.info(f"作者ID: {author['id']}")
                logger.info(f"{'='*60}\n")

                try:
                    articles, update_entries = (
                        self.article_extractor.get_author_articles(author)
                    )
                    total_articles += len(articles)
                    logger.info(
                        f"[完成] 作者 {author['name']} 获得 {len(articles)} 篇文章"
                    )

                    # all-history 模式下，如果一个作者一篇文章都没保存，说明出错了
                    if not self.update_today and len(articles) == 0:
                        logger.error(
                            f"[严重错误] all-history 模式下，作者 {author['name']} 一篇文章都未保存，可能哪里出错了"
                        )
                        logger.error(
                            "请检查：1) 网络连接 2) 登录状态 3) 雪球页面结构是否变化"
                        )
                        raise Exception(
                            f"all-history 模式下作者 {author['name']} 未抓取到任何文章"
                        )
            ...
            # ========== 清理资源 ==========
            # 只在登录成功且未发生登录验证错误时保存cookies
            driver = self.browser_manager.get_driver()
            # 验证登录状态，如果有效才保存cookies
            if self.author_extractor._verify_login_status(driver):
                logger.info("正在保存最新的 cookies...")
                self.cookie_handler.save_cookies(driver)
            else:
                logger.info("登录状态无效，跳过保存 cookies")
...
def main():
    ...
    scraper = ScraperOrchestrator(headless=headless, output_dir="/data/xueqiu_data")
