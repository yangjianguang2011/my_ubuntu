...
class CookieHandler:
    ...
    def _verify_login_status(self, driver: webdriver.Chrome) -> bool:
        ...
        try:
            ...
            if not any(keyword in title for keyword in ['login', '登录', 'signin']):
...
class AuthorExtractor:
    ...
    def _verify_login_status(self, driver: webdriver.Chrome) -> bool:
        ...
        try:
            ...
            logger.info(f"页面文本内容：{page_text[:500]}...")  # 输出前500字符以供调试

            title = driver.title.lower()
            logger.info(f"页面标题: {title}")
            if ("我的首页" in title) or ("个人中心" in title) or ("我的关注" in title):
                logger.info("登录状态验证成功 - 页面标题包含用户相关信息")
                return True
