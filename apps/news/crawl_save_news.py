...
def count_word_frequency(
    ...
    mode: str = "daily",  # mode is kept for potential future use but logic is simplified
) -> Tuple[List[Dict], int]:
    """按新闻来源对新闻进行分组和排序"""
    stats_by_source = {}
    total_titles = 0
    ...
    # Process all results, not results_to_process
    for source_id, titles_data in results.items():
        ...
        for title, data in titles_data.items():
            total_titles += 1
            # Get metadata from title_info if available
            info = title_info.get(source_id, {}).get(title, {})

            is_new = (
                new_titles
                and source_id in new_titles
                and title in new_titles[source_id]
            )

            title_details = {
                "title": title,
                "source_name": source_name,
                "ranks": data.get("ranks", [99]),
                "url": data.get("url", ""),
                ...
                "time_display": format_time_display(
                    info.get("first_time", ""), info.get("last_time", "")
                ),
                "is_new": is_new,
                "rank_threshold": rank_threshold,
            }
            stats_by_source[source_name]["titles"].append(title_details)

    stats = []
    for source_name, data in stats_by_source.items():
        # Sort titles within each source by rank
        sorted_titles = sorted(data["titles"], key=lambda x: min(x["ranks"]))
        data["titles"] = sorted_titles
        data["count"] = len(sorted_titles)
        stats.append(
            {
                "word": source_name,  # Use source_name as the group key
                "count": data["count"],
                "titles": data["titles"],
            }
        )

    # Sort sources by the number of news items, descending
    stats.sort(key=lambda x: x["count"], reverse=True)
    
    print(f"已按新闻来源分组，共 {len(stats)} 个来源, {total_titles} 条新闻。")
...
def prepare_report_data(
    ...
    """准备报告数据"""
    # The new UI design marks new items individually, so a separate "new titles" section is no longer needed.
    ...
    return {
        ...
        "new_titles": [],  # Always empty
        "failed_ids": failed_ids or [],
        "total_new_count": 0, # Always 0
    }
...
def generate_html_report(
    ...
    file_path = get_output_path(None, filename)
...
def render_html_content(
    ...
    """渲染HTML内容"""
    # Generate platform navigation HTML
    platforms_set = set()
    for stat in report_data["stats"]:
        source_name = stat["word"]
        platforms_set.add(source_name)
    
    platform_nav_html = ""
    if platforms_set:
        sorted_platforms = sorted(platforms_set)
        platform_links = []
        for platform_name in sorted_platforms:
            # Generate platform ID by replacing spaces with hyphens and converting to lowercase
            platform_id = platform_name.replace(' ', '-').replace('　', '-').lower()
            link_html = f'<a href="#{platform_id}" class="platform-link">{html_escape(platform_name)}</a>'
            platform_links.append(link_html)
        platform_nav_html = '\n'.join(platform_links)
    ...
    <head>
        ...
        <style>
            ...
            .platform-links {
                /* Change to flex-wrap to allow multi-line layout */
                display: flex;
                flex-wrap: wrap;
                gap: 10px;
                align-items: center;
            ...
            .platform-link {
                ...
                font-size: 14px;
                font-weight: 500;
                white-space: nowrap; /* Keep individual links from wrapping internally */
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>热点新闻</h1>
                <p>更新时间: """
    html += get_beijing_time().strftime("%Y-%m-%d %H:%M:%S")
    html += """</p>
            </div>
            <!-- Platform Navigation -->
    ...
    if report_data["stats"]:
        for stat in report_data["stats"]:
            source_name = html_escape(stat["word"])
            # Generate platform ID by replacing spaces with hyphens and converting to lowercase
            platform_id = source_name.replace(' ', '-').replace('　', '-').lower()
            html += f"""
            <div class="source-section" id="{platform_id}">
                <div class="source-header">
                    <h2>{source_name}</h2>
                </div>
                <ul class="news-list">
            """
            for i, title_data in enumerate(stat["titles"], 1):
                rank = min(title_data["ranks"])
                rank_class = "top-3" if rank <= 3 else ""
                title = html_escape(title_data["title"])
                url = html_escape(title_data.get("url") or title_data.get("mobile_url", ""))
                is_new = title_data.get("is_new", False)

                html += f"""
                    <li class="news-item">
                        <div class="news-rank {rank_class}">{i}</div>
        ...
        html += "<p style='text-align: center; padding: 20px;'>暂无新闻数据。</p>"
...
class NewsAnalyzer:
    ...
    def _crawl_data(self) -> Tuple[Dict, Dict, List]:
        ...
        # 不再保存为txt文件
        print(f"爬取完成，共获取 {len(results)} 个平台的数据")
    ...
    def _execute_mode_strategy(
        ...
        # 修复：在daily模式下，如果需要生成汇总报告，应该返回汇总HTML路径，而不是None
        if mode_strategy["should_generate_summary"] and mode_strategy["summary_mode"] == "daily":
