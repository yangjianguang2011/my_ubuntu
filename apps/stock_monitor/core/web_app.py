...
@app.route("/api/index/lppl_analysis_single", methods=["POST"])
def get_single_index_lppl_analysis_api():
    """获取单个指数的LPPL泡沫分析"""
    try:
        from ..analyzers.lppl_analyzer import analyze_index_with_lppl
        from ..data_fetchers.index_data_fetcher import get_index_daily_data

        # 获取请求参数
        data = request.json
        symbol = data.get("symbol")  # 指数代码
        name = data.get("name", "")  # 指数名称
        period = data.get("period", "3Y")  # 分析周期，默认3年

        if not symbol:
            logger.warning("请求中缺少指数代码")
            return jsonify({"success": False, "message": "请提供指数代码"}), 400

        logger.info(f"请求单个指数LPPL分析，指数: {symbol}({name}), 周期: {period}")
        ...
        try:
            ...
            if index_data.empty:
                ...
                return (
                    jsonify({"success": False, "message": f"指数 {symbol} 无历史数据"}),
                    400,
                )

            logger.info(f"获取到指数 {symbol} 的 {len(index_data)} 条历史数据")

            # 进行LPPL分析
            logger.info(f"开始对指数 {symbol} 进行LPPL分析...")
            lppl_result = analyze_index_with_lppl(index_data, name or symbol)

            logger.info(f"指数 {symbol} LPPL分析完成，成功: {lppl_result['success']}")
            ...
            return jsonify(
                {
                    ...
                    "symbol": symbol,
                    "name": name,
                    "message": f"成功分析指数 {symbol} 的LPPL特征",
                }
            )

        except Exception as e:
            logger.error(f"分析指数 {symbol} 时出错: {e}", exc_info=True)
            return (
                jsonify(
                    {"success": False, "message": f"分析指数 {symbol} 时出错: {str(e)}"}
                ),
                500,
            )

    except Exception as e:
        logger.error(f"获取单个指数LPPL分析失败: {str(e)}", exc_info=True)
        return (
            jsonify(
                {"success": False, "message": f"获取单个指数LPPL分析失败: {str(e)}"}
            ),
            500,
        )
