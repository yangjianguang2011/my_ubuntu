...
def get_sina_industry_names():
    ...
    try:
        ...
        industry_mask = spot_data["名称"].apply(
            lambda x: any(keyword in str(x) for keyword in industry_keywords)
        )
...
def get_industry_constituents(industry_name):
    ...
    try:
        ...
        set_industry_cache_data(
            cache_key, constituents, cache_duration=7 * 24 * 60 * 60
        )
...
def get_industry_ranking_original(period="30"):
    ...
    try:
        ...
        for industry in industry_list:
            ...
            try:
                ...
                if period == "1":
                    ...
                    recent_start_date = (current_date - timedelta(days=3)).strftime(
                        "%Y%m%d"
                    )
                    df = get_single_industry_history(
                        industry_name, recent_start_date, end_date
                    )
                else:
                    df = get_single_industry_history(
                ...
                if period == "1":
                    ...
                    if len(unique_dates) >= 2:
                        ...
                        period_data = df[df["日期"].isin(last_two_dates)].copy()
                ...
                ranking_data.append(
                    {
                        ...
                        "volume": (
                            float(last_record["成交量"])
                            if last_record["成交量"] and last_record["成交量"] != "-"
                            else 0
                        ),
                    }
                )
        ...
        if ranking_data:
            ranking_data.sort(
                key=lambda x: (x["change_pct"] is None, x["change_pct"]), reverse=True
            )
            set_industry_cache_data(cache_key, ranking_data)
            logger.info(
                f"获取到 {len(ranking_data)} 个行业的 {period}天 涨跌幅排行数据"
            )
        return ranking_data
    except Exception as e:
...
def get_sina_single_industry_history(industry_code, start_date, end_date):
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            logger.warning(
                f"行业 {industry_code} 返回的数据缺少必要列: {list(df.columns)}"
            )
            return pd.DataFrame()
...
def get_sina_industry_ranking(period="30"):
    ...
    try:
        ...
        for industry in industry_list:
            ...
            try:
                # 获取该行业的历史数据
                df = get_sina_single_industry_history(
                ...
                if period == "1":
                    ...
                    if len(unique_dates) >= 2:
                        # 获取最近两天的数据
                        last_two_dates = unique_dates[-2:]  # 最近的两个日期
                        period_data = df[df["date"].isin(last_two_dates)].copy()
                ...
                ranking_data.append(
                    {
                        ...
                        "volume": (
                            float(last_record["volume"])
                            if last_record["volume"] and last_record["volume"] != "-"
                            else 0
                        ),
                    }
                )
...
def get_single_industry_history(industry_name, start_date, end_date):
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            logger.warning(
                f"行业 {industry_name} 返回的数据缺少必要列: {list(df.columns)}"
            )
            return pd.DataFrame()
...
def get_industry_char_data_original(industry_names, period="365"):
    ...
    try:
        logger.info(
            f"开始获取 {len(industry_names)} 个行业的历史数据，周期: {period}天"
        )
        ...
        # 构建返回数据结构
        chart_data = {"dates": all_dates, "series": {}}
        ...
        for industry_name, df in industry_data_dict.items():
            ...
            for date in all_dates:
                if (
                    date in date_price_map
                    and date_price_map[date] is not None
                    and str(date_price_map[date]) != "None"
                ):
                    try:
                        ...
                        logger.warning(
                            f"行业 {industry_name} 日期 {date} 的收盘价无法转换为浮点数: {date_price_map[date]}"
                        )
                        continue
                    ...
                    if price is not None and str(price) != "None":
                        try:
                            ...
                            if base_price is not None and base_price != 0:
                                growth_rate = (
                                    (current_price - base_price) / base_price
                                ) * 100
                            ...
                            logger.warning(
                                f"行业 {industry_name} 日期 {date} 的收盘价无法转换为数字: {price}"
                            )
                            price_series.append(None)
                        ...
                        if base_price is not None and base_price != 0:
                            growth_rate = (
                                (last_valid_price - base_price) / base_price
                            ) * 100
                            price_series.append(round(growth_rate, 2))
                        else:
                            price_series.append(None)
