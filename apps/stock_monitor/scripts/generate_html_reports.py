...
logger = setup_logger("analyst_reports")

OUTPUT_DIR = get_path("crawlers", "analyst_data_dir", "/data/analyst_data")
...
def generate_analyst_report(top_analysts=20, top_stocks=50, period="3个月"):
    ...
    # 获取分析师数据
    analyst_data = get_analyst_combined_data(
...
def generate_recently_updated_stocks_report(days=30, indicator="最新跟踪成分股"):
    ...
    """
    logger.info(
        f"开始生成分析师最近更新股票报告，天数阈值: {days}天, 指标: {indicator}"
    )
    ...
    report_path = create_html_report(
        recently_updated_stocks, title, "recently_updated_stocks"
    )
