...
def display_table_data(conn, table_name, limit=10):
    ...
    if "cache_time" in column_names:
        cursor.execute(
            f"SELECT * FROM {table_name} ORDER BY cache_time DESC LIMIT {limit}"
        )
    else:
        # 如果没有cache_time列，按第一列排序（如果存在的话）
        if column_names:
            cursor.execute(
                f"SELECT * FROM {table_name} ORDER BY {column_names[0]} DESC LIMIT {limit}"
            )
        else:
    ...
    header = " | ".join(
        [f"{col_name[:15]:<15}" for col_name in header_names]
    )  # 限制列名长度
    print(header)
    print("-" * 100)
            ...
            if col_name == "data":
                try:
                    ...
                    if isinstance(data_obj, dict):
                        ...
                        if (
                            "price" in data_obj
                            or "change_pct" in data_obj
                            or "name" in data_obj
                        ):
                            formatted_data = []
                            for key, val in list(data_obj.items())[
                                :3
                            ]:  # 只显示前3个键值对
...
def query_by_cache_key(conn, cache_key, table_name):
    ...
    if "cache_key" in column_names:
        ...
        if found_key_column:
            ...
            print("-" * 100)
            cursor.execute(
                f"SELECT * FROM {table_name} WHERE {found_key_column} = ?", (cache_key,)
            )
        else:
            # 如果没有找到合适的键列，提示用户
            print(
...
def clear_table_data(conn, table_name):
    ...
    confirm = (
        input(f"确定要清空表 {table_name} 的所有数据吗？(输入 'yes' 确认): ")
        .strip()
        .lower()
    )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # 数据库路径
        db_dir = get_path(
            "stock_monitor", "database_dir", "../run/stock_monitor_data/database/"
        )
